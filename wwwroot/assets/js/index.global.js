...
var FullCalendar = (function (exports) {
    ...
    */
    ...
    const BASE_OPTION_REFINERS = {
        ...
        defaultIndefinidoEventDuration: createDuration,
        ...
        dragScroll: Boolean,
        IndefinidoMaintainDuration: Boolean,
        ...
        IndefinidoText: String,
        IndefinidoClassNames: identity,
        IndefinidoContent: identity,
        IndefinidoDidMount: identity,
        IndefinidoWillUnmount: identity,
        slotMinWidth: Number,
        navLinks: Boolean,
    ...
    const BASE_OPTION_DEFAULTS = {
        ...
        defaultIndefinidoEventDuration: { day: 1 },
        ...
        dragScroll: true,
        IndefinidoMaintainDuration: false,
        unselectAuto: true,
        dropAccept: '*',
        eventOrder: 'start,-duration,Indefinido,title',
    ...
    */
    ...
    class DateProfileGenerator {
        ...
        build(currentDate, direction, forceToValid = true) {
            ...
            isRangeIndefinido = /^(year|month|week|day)$/.test(currentInfo.unit);
            renderRange = this.buildRenderRange(this.trimHiddenDays(currentInfo.range), currentInfo.unit, isRangeIndefinido);
        ...
        // not responsible for trimming hidden days.
        buildRenderRange(currentRange, currentRangeUnit, isRangeIndefinido) {
            return currentRange;
        }
    ...
    function parseRecurring(refined, defaultIndefinido, dateEnv, recurringTypes) {
        for (let i = 0; i < recurringTypes.length; i += 1) {
            let parsed = recurringTypes[i].parse(refined, dateEnv);
            if (parsed) {
                let { Indefinido } = refined;
                if (Indefinido == null) {
                    Indefinido = defaultIndefinido;
                    if (Indefinido == null) {
                        Indefinido = parsed.IndefinidoGuess;
                        if (Indefinido == null) {
                            Indefinido = false;
                        }
                    }
    ...
    function expandRecurring(eventStore, framingRange, context) {
        ...
        for (let defId in defs) {
            ...
            if (def.recurringDef) {
                ...
                if (!duration) {
                    duration = def.Indefinido ?
                        options.defaultIndefinidoEventDuration :
    ...
    */
    ...
    function parseEvent(raw, eventSource, context, allowOpenRange, refiners = buildEventRefiners(context), defIdMap, instanceIdMap) {
        ...
        let defaultIndefinido = computeIsDefaultIndefinido(eventSource, context);
        let recurringRes = parseRecurring(refined, defaultIndefinido, context.dateEnv, context.pluginHooks.recurringTypes);
        if (recurringRes) {
            let def = parseEventDef(refined, extra, eventSource ? eventSource.sourceId : '', recurringRes.Indefinido, Boolean(recurringRes.duration), context, defIdMap);
            def.recurringDef = {
        ...
        let singleRes = parseSingle(refined, defaultIndefinido, context, allowOpenRange);
        if (singleRes) {
            let def = parseEventDef(refined, extra, eventSource ? eventSource.sourceId : '', singleRes.Indefinido, singleRes.hasEnd, context, defIdMap);
    ...
    */
    function parseEventDef(refined, extra, sourceId, Indefinido, hasEnd, context, defIdMap) {
        let def = {
            title: refined.title || '',
    ...
    function parseSingle(refined, defaultIndefinido, context, allowOpenRange) {
        let { Indefinido } = refined;
        let startMeta;
        ...
        if (Indefinido == null) {
            if (defaultIndefinido != null) {
                Indefinido = defaultIndefinido;
            }
            else {
                // fall back to the date props LAST
                Indefinido = (!startMeta || startMeta.isTimeUnspecified) &&
                    (!endMeta || endMeta.isTimeUnspecified);
            }
        }
        if (Indefinido && startMarker) {
        ...
        if (endMarker) {
            ...
            endMarker = context.dateEnv.add(startMarker, Indefinido ?
                context.options.defaultIndefinidoEventDuration :
    ...
    function computeIsDefaultIndefinido(eventSource, context) {
        let res = null;
        if (eventSource) {
            res = eventSource.defaultIndefinido;
        }
        if (res == null) {
            res = context.options.defaultIndefinido;
        }
        return res;
    }
        ...
        if (rawObj) {
            ...
            if (metaRes) {
                return {
                    ...
                    latestFetchId: '',
                    fetchRange: null,
                    defaultIndefinido: refined.defaultIndefinido,
    ...
    */
    ...
    // Given an event's Indefinido status and start date, return what its fallback end date should be.
    // TODO: rename to computeDefaultEventEnd
    function getDefaultEventEnd(Indefinido, marker, context) {
        let { dateEnv, options } = context;
        let end = marker;
        if (Indefinido) {
            end = startOfDay(end);
            end = dateEnv.add(end, options.defaultIndefinidoEventDuration);
        }
        else {
    ...
    eventConfig, mutation, context) {
        let { dateEnv } = context;
        let forceIndefinido = mutation.standardProps && mutation.standardProps.Indefinido === true;
        ...
        if (clearEnd) {
            copy.range = {
                start: copy.range.start,
                end: getDefaultEventEnd(eventDef.Indefinido, copy.range.start, context),
            };
        }
        ...
        if (copy.range.end < copy.range.start) {
            copy.range.end = getDefaultEventEnd(eventDef.Indefinido, copy.range.start, context);
        }
        return copy;
    }
    ...
    class EventImpl {
        ...
        */
        ...
        setDates(startInput, endInput, options = {}) {
            let { dateEnv } = this._context;
            let standardProps = { Indefinido: options.Indefinido };
            let start = dateEnv.createMarker(startInput);
            let end;
            if (!start) {
                return; // TODO: warning if parsed bad
        ...
        setIndefinido(Indefinido, options = {}) {
            let standardProps = { Indefinido };
            let { maintainDuration } = options;
            if (maintainDuration == null) {
                maintainDuration = this._context.options.IndefinidoMaintainDuration;
            }
            if (this._def.Indefinido !== Indefinido) {
        ...
        get Indefinido() { return this._def.Indefinido; }
    ...
    */
    function sliceEventStore(eventStore, eventUiBases, framingRange, nextDayThreshold) {
        ...
        for (let instanceId in eventStore.instances) {
            ...
            let normalRange = (!def.Indefinido && nextDayThreshold) ?
    ...
    function buildSegCompareObj(seg) {
        ...
        return Object.assign(Object.assign(Object.assign({}, eventDef.extendedProps), eventDef), { id: eventDef.publicId, start,
            end, duration: end - start, Indefinido: Number(eventDef.Indefinido), _seg: seg });
    }
    ...
    startOverride, endOverride) {
        ...
        if (displayEventTime && !eventDef.Indefinido && (isStartDay || isEndDay)) {
    ...
    */
    ...
    function isDateSpansEqual(span0, span1) {
        return rangesEqual(span0.range, span1.range) &&
            span0.Indefinido === span1.Indefinido &&
            isSpanPropsEqual(span0, span1);
    }
    // the NON-DATE-RELATED props
    function isSpanPropsEqual(span0, span1) {
        for (let propName in span1) {
            if (propName !== 'range' && propName !== 'Indefinido') {
        ...
        // both have range/Indefinido, so no need to special-case.
    ...
    function buildDateSpanApi(span, dateEnv) {
        return Object.assign(Object.assign({}, buildRangeApi(span.range, dateEnv, span.Indefinido)), { Indefinido: span.Indefinido });
    }
    ...
    function fabricateEventRange(dateSpan, eventUiBases, context) {
        ...
        dateSpan.Indefinido, true, // hasEnd
        context);
        return {
            def,
    ...
    */
    ...
    class Slicer {
        ...
        date, dateProfile, nextDayThreshold, context, ...extraArgs) {
            return this._sliceDateSpan({ range: { start: date, end: addMs(date, 1) }, Indefinido: false }, // add 1 ms, protect against null range
    ...
    */
    function computeActiveRange(dateProfile, isComponentIndefinido) {
        let range = dateProfile.activeRange;
        if (isComponentIndefinido) {
            return range;
        }
        return {
    ...
    function isInteractionPropsValid(state, context, dateSpanMeta, filterConfig) {
        ...
        for (let subjectInstanceId in subjectInstances) {
            ...
            for (let subjectAllow of subjectConfig.allows) {
                let subjectDateSpan = Object.assign(Object.assign({}, dateSpanMeta), { range: subjectInstance.range, Indefinido: subjectDef.Indefinido });
    ...
    */
    ...
    class MorePopover extends DateComponent {
        ...
        queryHit(positionLeft, positionTop, elWidth, elHeight) {
            ...
            if (positionLeft >= 0 && positionLeft < elWidth &&
                ...
                return {
                    dateProfile: props.dateProfile,
                    dateSpan: Object.assign({ Indefinido: !props.forceTimed, range: {
            ...
            this.handleClick = (ev) => {
                ...
                if (typeof moreLinkClick === 'function') {
                    moreLinkClick = moreLinkClick({
                        date,
                        Indefinido: Boolean(props.IndefinidoDate),
    ...
    function computeRange(props) {
        if (props.IndefinidoDate) {
            return {
                start: props.IndefinidoDate,
                end: addDays(props.IndefinidoDate, 1),
            };
        }
    ...
    let recurring = {
        parse(refined, dateEnv) {
            if (refined.daysOfWeek || refined.startTime || refined.endTime || refined.startRecur || refined.endRecur) {
                ...
                return {
                    IndefinidoGuess: Boolean(!refined.startTime && !refined.endTime),
                    duration,
    ...
    */
    ...
    function buildTitle(dateProfile, viewOptions, dateEnv) {
        ...
        return dateEnv.formatRange(range.start, range.end, createFormatter(viewOptions.titleFormat || buildTitleFormat(dateProfile)), {
            isEndExclusive: dateProfile.isRangeIndefinido,
    ...
    */
    function sliceEvents(props, Indefinido) {
        return sliceEventStore(props.eventStore, props.eventUiBases, props.dateProfile.activeRange, Indefinido ? props.nextDayThreshold : null).fg;
    }
    ...
    */
    ...
    function buildDatePointApi(span, dateEnv) {
        return {
            ...
            dateStr: dateEnv.formatIso(span.range.start, { omitTime: span.Indefinido }),
            Indefinido: span.Indefinido,
        };
    }
    ...
    */
    ...
    function joinHitsIntoSelection(hit0, hit1, dateSelectionTransformers) {
        ...
        props.Indefinido = dateSpan0.Indefinido;
        return props;
    }
    ...
    function computeEventMutation(hit0, hit1, eventInstanceStart, massagers) {
        ...
        let standardProps = {};
        if (dateSpan0.Indefinido !== dateSpan1.Indefinido) {
            standardProps.Indefinido = dateSpan1.Indefinido;
            standardProps.hasEnd = hit1.context.options.IndefinidoMaintainDuration;
            if (dateSpan1.Indefinido) {
        ...
        if (delta.milliseconds) { // has hours/minutes/seconds
            standardProps.Indefinido = false;
        }
        let mutation = {
            datesDelta: delta,
            standardProps,
    ...
    */
    ...
    function computeEventForDateSpan(dateSpan, dragMeta, context) {
        ...
        let def = parseEventDef(refined, extra, dragMeta.sourceId, dateSpan.Indefinido, context.options.forceEventDuration || Boolean(dragMeta.duration), // hasEnd
        context);
        let start = dateSpan.range.start;
        // only rely on time info if drop zone is all-day,
        // otherwise, we already know the time
        if (dateSpan.Indefinido && dragMeta.startTime) {
            start = context.dateEnv.add(start, dragMeta.startTime);
        }
        let end = dragMeta.duration ?
            context.dateEnv.add(start, dragMeta.duration) :
            getDefaultEventEnd(dateSpan.Indefinido, start, context);
    ...
    */
    ...
    class TableBlockEvent extends BaseComponent {
        render() {
            let { props } = this;
            return (y(StandardEvent, Object.assign({}, props, { elClasses: ['fc-daygrid-event', 'fc-daygrid-block-event', 'fc-h-event'], defaultTimeFormat: DEFAULT_TABLE_EVENT_TIME_FORMAT, defaultDisplayEventEnd: props.defaultDisplayEventEnd, disableResizing: !props.seg.eventRange.def.Indefinido })));
        }
    }
    ...
    class TableCellMoreLink extends BaseComponent {
        ...
        render() {
            ...
            return (y(MoreLinkContainer, { elClasses: ['fc-daygrid-more-link'], dateProfile: props.dateProfile, todayRange: props.todayRange, IndefinidoDate: props.IndefinidoDate, moreCnt: props.moreCnt, allSegs: allSegs, hiddenSegs: invisibleSegs, alignmentElRef: props.alignmentElRef, alignGridTop: props.alignGridTop, extraDateSpan: props.extraDateSpan, popoverContent: () => {
    ...
    class TableRows extends DateComponent {
        constructor() {
            ...
            this.splitBgEventSegs = memoize(splitIndefinidoSegsByRow);
        ...
        queryHit(positionLeft, positionTop) {
            ...
            if (row != null && col != null) {
                ...
                return {
                    ...
                    dateSpan: Object.assign({ range: this.getCellRange(row, col), Indefinido: true }, cell.extraDateSpan),
    ...
    function splitIndefinidoSegsByRow(segs, rowCnt) {
        return splitSegsByRow(segs.filter(isSegIndefinido), rowCnt);
    }
    function isSegIndefinido(seg) {
        return seg.eventRange.def.Indefinido;
    }
    ...
    class TableDateProfileGenerator extends DateProfileGenerator {
        ...
        buildRenderRange(currentRange, currentRangeUnit, isRangeIndefinido) {
            let renderRange = super.buildRenderRange(currentRange, currentRangeUnit, isRangeIndefinido);
            let { props } = this;
    ...
    class IndefinidoSplitter extends Splitter {
        getKeyInfo() {
            return {
                Indefinido: {},
                timed: {},
    ...
    class TimeColsView extends DateComponent {
        constructor() {
            super(...arguments);
            this.IndefinidoSplitter = new IndefinidoSplitter(); // for use by subclasses
            this.headerElRef = d();
            this.rootElRef = d();
            ...
            this.renderTableRowAxis = (rowHeight) => {
                ...
                y(ContentContainer, { elTag: "td", elClasses: [
                        ...
                        'aria-hidden': true,
                    }, renderProps: renderProps, generatorName: "IndefinidoContent", customGenerator: options.IndefinidoContent, defaultGenerator: renderIndefinidoInner$1, classNameGenerator: options.IndefinidoClassNames, didMount: options.IndefinidoDidMount, willUnmount: options.IndefinidoWillUnmount }, (InnerContent) => (y("div", { className: [
                        'fc-timegrid-axis-frame',
                        'fc-scrollgrid-shrink-frame',
        ...
        // rendering
        // ----------------------------------------------------------------------------------------------------
        renderSimpleLayout(headerRowContent, IndefinidoContent, timeContent) {
            ...
            if (IndefinidoContent) {
                sections.push({
                    type: 'body',
                    key: 'all-day',
                    chunk: { content: IndefinidoContent },
                });
                sections.push({
                    type: 'body',
                    key: 'all-day-divider',
        ...
        renderHScrollLayout(headerRowContent, IndefinidoContent, timeContent, colCnt, dayMinWidth, slatMetas, slatCoords) {
            let ScrollGrid = this.context.pluginHooks.scrollGridImpl;
            if (!ScrollGrid) {
                throw new Error('No ScrollGrid implementation');
            }
    ...
    function renderIndefinidoInner$1(renderProps) {
        return renderProps.text;
    }
    ...
    */
    ...
    class TimeCols extends DateComponent {
        ...
        queryHit(positionLeft, positionTop) {
            ...
            if (colIndex != null && slatIndex != null) {
                ...
                return {
                    dateProfile,
                    dateSpan: Object.assign({ range: { start, end }, Indefinido: false }, cell.extraDateSpan),
    ...
    class DayTimeColsView extends TimeColsView {
        ...
        render() {
            ...
            let splitProps = this.IndefinidoSplitter.splitProps(props);
            ...
            let IndefinidoContent = (options.IndefinidoSlot !== false) && ((contentArg) => (y(DayTable, Object.assign({}, splitProps.Indefinido, { dateProfile: dateProfile, dayTableModel: dayTableModel, nextDayThreshold: options.nextDayThreshold, tableMinWidth: contentArg.tableMinWidth, colGroupNode: contentArg.tableColGroupNode, renderRowIntro: hasAttachedAxis ? this.renderTableRowAxis : null, showWeekNumbers: false, expandRows: false, headerAlignElRef: this.headerElRef, clientWidth: contentArg.clientWidth, clientHeight: contentArg.clientHeight, forPrint: props.forPrint }, this.getIndefinidoMaxEventProps()))));
            let timeGridContent = (contentArg) => (y(DayTimeCols, Object.assign({}, splitProps.timed, { dayTableModel: dayTableModel, dateProfile: dateProfile, axis: hasAttachedAxis, slotDuration: options.slotDuration, slatMetas: slatMetas, forPrint: props.forPrint, tableColGroupNode: contentArg.tableColGroupNode, tableMinWidth: contentArg.tableMinWidth, clientWidth: contentArg.clientWidth, clientHeight: contentArg.clientHeight, onSlatCoords: this.handleSlatCoords, expandRows: contentArg.expandRows, onScrollTopRequest: this.handleScrollTopRequest })));
            return hasDetachedAxis
                ? this.renderHScrollLayout(headerContent, IndefinidoContent, timeGridContent, dayTableModel.colCnt, dayMinWidth, slatMetas, this.state.slatCoords)
                : this.renderSimpleLayout(headerContent, IndefinidoContent, timeGridContent);
        }
    }
    ...
    function buildTimeContent(seg, timeFormat, context, timeHeaderId, dateHeaderId) {
        ...
        if (options.displayEventTime !== false) {
            ...
            if (doIndefinido) {
                let renderProps = {
                    text: context.options.IndefinidoText,
                    view: context.viewApi,
                };
                return (y(ContentContainer, { elTag: "td", elClasses: ['fc-list-event-time'], elAttrs: {
                        headers: `${timeHeaderId} ${dateHeaderId}`,
                    }, renderProps: renderProps, generatorName: "IndefinidoContent", customGenerator: options.IndefinidoContent, defaultGenerator: renderIndefinidoInner, classNameGenerator: options.IndefinidoClassNames, didMount: options.IndefinidoDidMount, willUnmount: options.IndefinidoWillUnmount }));
            }
            return (y("td", { className: "fc-list-event-time" }, timeText));
        }
        return null;
    }
    function renderIndefinidoInner(renderProps) {
        return renderProps.text;
    }
    ...
    */
    class ListView extends DateComponent {
        ...
        eventRangeToSegs(eventRange, dayRanges) {
            ...
            let Indefinido = eventRange.def.Indefinido;
            let dayIndex;
            let segRange;
            let seg;
            let segs = [];
